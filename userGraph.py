...
if __name__ == '__main__':
    ...
    tweet_df = pd.read_csv("D:\\edu\\catogory\data_files\\no_filter\\tweet_topic_per_user.csv", sep=' ')
    ...
    for row in tweet_df.iterrows():
        ...
        user_occurrence = [0] * size
        for topic in row['text'].split(','):
            topic = int(topic)
            if user_occurrence[topic] == 0:
                if row['label'] == 1:
                    normal_users[topic] += 1
                elif row['label'] == 0:
                    auti[topic] += 1
                user_occurrence[topic] = 1


    list_of_point = []
    whole = [sum(x) for x in zip(normal_users, auti)]
    print(type(whole))
    ...
    for inx, item in enumerate(list_of_point):
        if inx % 2.0 == 0:  # if it is even
            ...
            transformed_list_normal[inx] = normal_users[item.index]
        else:
            print(size_plus, inx)
            transformed_list_auti[size_plus - inx] = auti[item.index]
            transformed_list_normal[size_plus - inx] = normal_users[item.index]
